...
public class CommandEncoder
{
    ...
    private static int prevVideoType = -11;
    public static void parseMediaCommandV3(WifiCar wificar, byte[] buf, int i) throws IOException {
        int k = findstr(buf, i, "MO_V");
        ...
        if (k  >= 0) {
            ...
            if (mediaRecvBuf != null && mediaRecvBuf.length > 0) {
                ...
                int currentVideoType = (int)buf[31];
                if (prevVideoType != currentVideoType) {
                    //uinit & int
                    wificar.reInitH264So();
                    prevVideoType = currentVideoType;
                }
                int videoLen = byteArrayToInt(buf, 32, 4);
                wificar.refreshH264View(arrayCopy(mediaRecvBuf, 36, mediaRecvBuf.length - 36),
                        mediaRecvBuf.length - 36, currentVideoType);
                AppLog.i(TAG, "parseMediaCommandV3:refresh 1 h264 frame siez:"+ (mediaRecvBuf.length - 36) + " iLen:" + videoLen);
                mediaRecvBuf = arrayCopy(buf, k, i/*buf.length*/ - k);

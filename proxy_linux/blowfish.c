...
If you require this code under a license other than LGP, please ask.  (I 
...
static const uint32_t ORIG_S[4][256] = { {
                ...
                0xff340528, 0xc5855664, 0x53b02d5d, 0xa99f8fa1, 0x8ba4799, 0x6e85076a},
            {
                ...
                0x3d28f89e, 0xf16dff20, 0x153e21e7, 0x8fb03d4a, 0xe6e39f2b, 0xdb83adf7},
             {
                ...
                0x5366f9c3, 0xc8b38e74, 0xb475f255, 0x46fcd9b9, 0x7aeb2661, 0x8b1ddf84, 0x846a0e79, 0x915f958e, 0x466e598e, 0x20b45770, 
...
void Blowfish_Init(BLOWFISH_CTX *ctx, uint8_t *key, int32_t keyLen) {
  ...
  DBG("Blowfish_Init1,res S:%u;%u;%u;%u \n", ctx->S[0][0],ctx->S[1][0], ctx->S[2][0], ctx->S[3][0]);
  int x = 0xF8107003;
  uint16_t a, b, c, d;
  uint32_t  y;
  d = (uint16_t)(x & 0xFF);
   ...
   DBG("Blowfish_Init0,ABCD:%d;%d;%d;%d;%u \n", a,b, c, d,y);
  j = 0;
  DBG("Blowfish_Init2,res P:\n");
  for (i = 0; i < N + 2; ++i) {
    data = 0x00000000;
    for (k = 0; k < 4; ++k) {
      data = (data << 8) | key[j];
  ...
  for (i = 0; i < N + 2; i += 2) {
    ...
    ctx->P[i] = datal;
    ctx->P[i + 1] = datar;
    DBG("Blowfish_Init,round:%d L:%u R:%u \n", i, datal, datar);
  }
  DBG("Blowfish_Init3,res P:%u;%u;%u;%u \n", ctx->P[0],ctx->P[1], ctx->P[16], ctx->P[17]);
  for (i = 0; i < 4; ++i) {
    for (j = 0; j < 256; j += 2) {
      Blowfish_Encrypt(ctx, &datal, &datar);
      ctx->S[i][j] = datal;
  ...
  DBG("Blowfish_Init4,res S:%u;%u;%u;%u \n", ctx->S[0][0],ctx->S[1][0], ctx->S[2][0], ctx->S[3][0]);
